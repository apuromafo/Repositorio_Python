const Renderer = {
    ...
    renderOptions: (q) => {
        ...
        if (q.type === 'likert') {
            Renderer.renderLikert(q, container);
            return;
        }
    ...
    renderLikert: (q, container) => {
        ...
        q.options.forEach((opt, i) => {
            ...
            val.style.cssText = 'font-size:.75rem;color:var(--text-muted);margin-left:8px;';
            val.innerText = scale.length > i ? `(${scale[i]})` : '';

            const input = document.createElement('input');
            input.style.display = 'none';
            input.type = 'radio';
            input.name = 'likert-opt';
            input.checked = isSelected;
            input.onchange = () => {
                AppState.answers[AppState.currentIndex] = i;
                Renderer.renderOptions(q);
